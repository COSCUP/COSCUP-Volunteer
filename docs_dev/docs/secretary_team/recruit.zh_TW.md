...
??? Note "2023 預期目標"
    ...
    如果今年的工作人員住宿分布在多個旅宿的情況下，會需要增加多個停靠站來提供接駁。
...
??? Note "2023 預期目標"
    ...
    目前也在持續增補財務組的預算、報帳流程。今年希望能推出**行動裝置版本**（預計使用 [Flutter](https://zh.wikipedia.org/wiki/Flutter)），方便大會活動期間能操作流暢。
...
??? Note "2023 預期目標"
    ...
    新增可以直接透過 OAuth 方式登入並完成訂閱流程。（使用者常常打錯郵件信箱。例如：`gmail.con`, `gmali.com`, `gmail.com.tw`）
