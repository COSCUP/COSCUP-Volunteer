                    ...
                    [
                        {
                            ...
                            u'驗證（verify）',
                            'url':
                            'https://%s/telegram/verify/%s' %
                            (setting.DOMAIN, uuid_data['uuid'])
                        },
                    ],
                ]
            },
    ...
    if request.method == 'GET':
        ...
        if data and user_data:
            ...
            r = TelegramBot(token=setting.TELEGRAM_TOKEN).send_message(
                ...
                text=
                u'與 [%(uid)s](https://volunteer.coscup.org/user/%(uid)s) 完成帳號綁定！（Completed）'
                % save_data)

            mc.delete_multi(
                ['tg:%s' % tg_uuid,
                 'tgu:%s' % g.user['account']['_id']])

            logging.info('[Telegram][Send] linkme: %(id)s %(uid)s' % save_data)

            return redirect(
                url_for('setting.link_telegram',
                        _scheme='https',
                        _external=True))
