...
@app.task(bind=True,
          name='ipinfo.update.usession',
          autoretry_for=(Exception, ),
          retry_backoff=True,
          max_retries=5,
          routing_key='cs.ipinfo.update.usession',
          exchange='COSCUP-SECRETARY')
def ipinfo_update_usession(sender, **kwargs):
    for u in USession.get_no_ipinfo():
        ipinfo_update_usession_one.apply_async(kwargs={
            'ip': u['header']['X-Real-Ip'],
            'sid': u['_id']
        })


@app.task(bind=True,
          name='ipinfo.update.usession.one',
          autoretry_for=(Exception, ),
          retry_backoff=True,
          max_retries=5,
          routing_key='cs.ipinfo.update.usession.one',
          exchange='COSCUP-SECRETARY')
