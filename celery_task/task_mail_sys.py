...
@app.task(bind=True,
          name='mail.sys.weberror',
          autoretry_for=(Exception, ),
          retry_backoff=True,
          max_retries=5,
          routing_key='cs.mail.sys.weberror',
          exchange='COSCUP-SECRETARY')
    ...
    for raw in team_member_change_db.find(
        ...
        for uid in team['chiefs']:
            ...
            raw_mail = awsses.raw_mail(
                ...
                subject=u'申請加入通知信 - %s' %
                users[raw['uid']]['profile']['badge_name'],
                body=body,
            )
            ...
            if mid:
                ...
                r = mmt.posts(
                    ...
                    message=
                    u'收到 **%s** 申請加入 **%s**，前往 [管理組員](https://volunteer.coscup.org/team/%s/%s/edit_user)'
                    % (users[raw['uid']]['profile']['badge_name'],
                       team['name'], team['pid'], team['tid']))
                logger.info(r.json())


@app.task(bind=True,
          name='mail.member.deny',
          autoretry_for=(Exception, ),
          retry_backoff=True,
          max_retries=5,
...
@app.task(bind=True,
          name='mail.member.welcome',
          autoretry_for=(Exception, ),
          retry_backoff=True,
          max_retries=5,
          routing_key='cs.mail.member.welcom',
          exchange='COSCUP-SECRETARY')
...
@app.task(bind=True,
          name='mail.member.send',
          autoretry_for=(Exception, ),
          retry_backoff=True,
          max_retries=5,
          routing_key='cs.mail.member.send',
          exchange='COSCUP-SECRETARY')
...
@app.task(bind=True,
          name='mail.tasks.star',
          autoretry_for=(Exception, ),
          retry_backoff=True,
          max_retries=5,
          routing_key='cs.mail.tasks.star',
          exchange='COSCUP-SECRETARY')
...
@app.task(bind=True,
          name='mail.tasks.star.one',
          autoretry_for=(Exception, ),
          retry_backoff=True,
          max_retries=5,
          routing_key='cs.mail.tasks.star.one',
          exchange='COSCUP-SECRETARY')
