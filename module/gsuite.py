...
class GSuite(object):
    ...
    def users_list(self):
        return self.service.users().list(customer='my_customer',
    ...
    def groups_list_loop(self, page_token=None):
        ...
        if 'nextPageToken' in groups:
            for group in self.groups_list_loop(
                    page_token=groups['nextPageToken']):
                yield group
    ...
    def members_list(self, group_key, page_token=None):
        ''' members.list '''
        return self.service.members().list(groupKey=group_key,
                                           pageToken=page_token).execute()
    ...
    def members_list_loop(self, group_key):
        ...
        while 'nextPageToken' in members:
            members = self.members_list(group_key,
                                        page_token=members['nextPageToken'])
            for member in members.get('members', []):
                yield member

    def members_insert(self,
                       group_key,
                       email,
                       role='MEMBER',
        ...
        return self.service.members().insert(groupKey=group_key,
                                             body=body).execute()
    ...
    def members_get(self, group_key, email):
        ''' members.get '''
        return self.service.members().get(groupKey=group_key,
                                          memberKey=email).execute()

    def members_delete(self, group_key, email):
        ''' members.delete '''
        return self.service.members().delete(groupKey=group_key,
                                             memberKey=email).execute()
